BFit<-function(x){
  ...
  X10R = as.numeric(BData[BData$Treatment=="Rhizosphere", c("DNAinit")])[1]/x[7]
  X10 = as.numeric(BData[BData$Treatment!="Rhizosphere", c("DNAinit")])[1]/x[7]
  E0 = x[4]*x[5]/x[1]/x[3]
  ...
  #==========================Running simulation
  Yhat <- rbind(Blag2014ODESolv(DEBmodel, x, timesR, y0R),
                Blag2014ODESolv(DEBmodel, x, times, y0))
  Sim <- as.data.frame(rbind(Blag2014ODESolv(DEBmodel, x, timesSim, y0R),
                             Blag2014ODESolv(DEBmodel, x, timesSim, y0)))
