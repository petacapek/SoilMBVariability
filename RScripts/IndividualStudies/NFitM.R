NFitM<-function(x){
  ...
  B0A = with(subset(NData, Treatment == "A"), ATPinit/x[6])[1]
  y0A <- c(as.numeric(NData[NData$Treatment=="A", "Sinit"])[1], B0A, 0)
  B0B = with(subset(NData, Treatment == "B"), ATPinit/x[6])[1]
  y0B <- c(as.numeric(NData[NData$Treatment=="B", "Sinit"])[1], B0B, 0)
  B0C = with(subset(NData, Treatment == "C"), ATPinit/x[6])[1]
  y0C <- c(as.numeric(NData[NData$Treatment=="C", "Sinit"])[1], B0C, 0)
  #==========================Running simulation
  Yhat <- rbind(Nanni1977ODESolvM(Monod, x, times, y0A),
                Nanni1977ODESolvM(Monod, x, times, y0B),
                Nanni1977ODESolvM(Monod, x, times, y0C))
  Sim <- as.data.frame(rbind(Nanni1977ODESolvM(Monod, x, timesSim, y0A),
                             Nanni1977ODESolvM(Monod, x, timesSim, y0B),
                             Nanni1977ODESolvM(Monod, x, timesSim, y0C)))
