TFitP<-function(x){
  ...
  B0 = TData$Cmicinit[1]/x[6]
  y0H <- c(as.numeric(TData[TData$Treatment=="HG", c("Sinit")])[1], B0, 0)
  y0L <- c(as.numeric(TData[TData$Treatment!="HG", c("Sinit")])[1], B0, 0)
  #==========================Running simulation
  Yhat <- rbind(Tsai1997ODESolvP(Pirt, x, times, y0H), 
                Tsai1997ODESolvP(Pirt, x, times, y0L))       
  Sim <- as.data.frame(rbind(Tsai1997ODESolvP(Pirt, x, timesSim, y0H), 
               Tsai1997ODESolvP(Pirt, x, timesSim, y0L)))
