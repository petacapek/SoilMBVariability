BKFit<-function(x){
  ...
  times <- as.numeric(BKData[BKData$Treatment=="HC", c("Time")])
  ...
  Sim <- as.data.frame(rbind(Bremer1990ODESolv(DEBmodelIso, x, timesSim, y0HC),
                       Bremer1990ODESolv(DEBmodelIso, x, timesSim, y0HN)))
  colnames(Sim) <- c("S", "CO2", "kec", "Cmic14")
  Sim$Time <- rep(timesSim, 2)
  Sim$Treatment <- rep(c("HC", "HCHN"), each = length(timesSim))
